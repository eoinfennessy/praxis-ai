...
impl SearchClient {
    ...
    async fn execute_search_with_response_limit(
        ...
        self.map_callout_outcome(result, aggregate_constrained)
    }
    ...
    fn map_callout_outcome(
        &self,
        result: Result<CalloutOutcome, FilterError>,
        aggregate_constrained: bool,
    ) -> SearchOutcome {
        match result {
            Ok(outcome) => self.map_callout_success(outcome, aggregate_constrained),
            Err(e) => {
...
mod tests {
    ...
    fn aggregate_constrained_response_limit_is_distinct() {
        let client = test_search_client();
        let outcome = client.map_callout_success(
            CalloutOutcome::ResponseTooLarge {
                actual: Some(4_097),
                limit: 4_096,
            },
            true,
