...
impl FileSearchError {
    ...
    fn staging_bytes(&self) -> Option<usize> {
        match self {
            ...
            Self::Deserialize { store_id, .. } | Self::AggregateLimit { store_id, .. } => Some(store_id.len()),
        }
    }
}
...
impl FileSearchClient {
    ...
    pub async fn search(
        ...
        self.search_with_retained_limit(specs, call_count, request_headers, usize::MAX)
            .await
    }
    ...
    #[expect(
        ...
        reason = "deadline and two independent response admissions stay explicit"
    )]
    async fn search_one(
        &self,
        spec: &SearchSpec<'_>,
...
fn merge_search_result(
    ...
    let body_bytes = match &response {
        ...
        Err(FileSearchError::Callout { .. } | FileSearchError::AggregateLimit { .. }) => 0,
    };
...
mod tests {
    ...
    use super::{
        FileSearchError, SearchBatch, SearchFailure, SearchResult, SearchSpec, VectorStoreSearchRequest,
        append_unprocessed_deadline_failures, deserialize_search_results, merge_top_results, parse_response_body,
        response_admission_units, retained_response_body_limit,
    };
